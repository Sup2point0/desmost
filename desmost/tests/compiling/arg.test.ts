...
describe("enum literals", () =>
{
	...
	test.each([
		...
		`/label{pos: RIGHT, text: "Test"} :: (0, 0)`,
		`/label{text: "Test", pos: RIGHT} :: (0, 0)`,
	])
	("with commas", compiles);

	test.each([
		`/line{style: DOTTED} /label{pos: RIGHT, text: "Test"} :: f(x)`,
		`/line{style: DOTTED} /label{text: "Test", pos: RIGHT} :: f(x)`,
	])
