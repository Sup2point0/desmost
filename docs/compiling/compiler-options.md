...
| **ignore_comments** | `true` `false` | `false`, meaning comments are kept. | Should comments be ignored, instead of turned into text expressions (notes)? |
| **ignore_blank_lines** | `true` `false` | `false`, meaning all blank lines are kept. | Should all line breaks be ignored, instead of kept as blank expressions? |
